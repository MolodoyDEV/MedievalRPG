...
namespace QuantumTek.QuantumQuest
{
    ...
    public class QQ_QuestHandler : MonoBehaviour
    {
        ...
        public List<string> S_Quests = new List<string>();

        /// <summary>
        /// Assigns a quest from the database to this quest handler.
        /// </summary>
        /// <param name="name">The name of the quest.</param>
        public QQ_Quest AssignQuest(string name)
        {
            if (Quests.TryGetValue(name, out QQ_Quest quest) == false)
            {
                //quest = new QQ_Quest(questDB.GetQuest(name));
                quest = questDB.GetQuest(name);
        ...
        public QQ_Quest AssignAndActivateQuest(string name)
        {
            QQ_Quest quest = AssignQuest(name);
            quest.Status = QQ_QuestStatus.Active;
            return quest;
        }
        ...
        public QQ_Quest GetQuest(string name)
        {
            ...
            throw new NullReferenceException($"Quest {name} not found");
        }
